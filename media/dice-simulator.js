(() => {
  ...
  coinImage.src=assetPrefix+'plinko-coins.webp';coinImage.alt=lang==='ru'?'Виртуальные монеты':'Virtual coins';
  ...
  otherGames.innerHTML=`<div class="dice-other-head"><h2>${extra.title}</h2><p>${extra.intro}</p></div><div class="dice-other-grid"><a class="dice-other-card" href="simulator-plinko.html"><img src="${assetPrefix}plinko-card-hero.webp" alt="Plinko"><div><span>${extra.available}</span><h3>${extra.plinko}</h3><p>${extra.plinkoText}</p><strong>${extra.play}</strong></div></a><a class="dice-other-card" href="simulator-limbo.html"><img src="${assetPrefix}limbo-card-hero.webp" alt="Limbo"><div><span>${extra.available}</span><h3>${extra.limbo}</h3><p>${extra.limboText}</p><strong>${extra.play}</strong></div></a></div>`;
